...
def main():
    ...
    parser.add_argument(
        '--stimulus', type=str, required=True,
        choices=['text', 'ran', 'hss', 'fxs', 'video', 'all'],
    )
    ...
    parser.add_argument('--fix_window_size', type=int, default=100)
    parser.add_argument(
        '--scanpath_model', type=str,
        choices=['random', 'stat_model'],
    )
    ...
    print('save data')
    if window_size != 5000:
        data_save_path = data_dir + 'synthetic_data_' + str(stimulus) + '_' + str(scanpath_model) + '_' + str(output_window_size) + data_suffix
